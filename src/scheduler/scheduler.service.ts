...
export class SchedulerService {
  ...
  async trackFeedbacks() {
    const settings = await this.settingsService.getSettings();

    if (settings.trackNewFeedbacks) {
  ...
  async trackReturnClaims() {
    const settings = await this.settingsService.getSettings();

    if (settings.trackNewClaims) {
  ...
  async trackNewChatMessages() {
    const settings = await this.settingsService.getSettings();

    if (settings.trackNewChatMessages) {
  ...
  async trackTokenExpiration() {
    const settings = await this.settingsService.getSettings();

    if (settings.trackTokenExpiration) {
